...
use super::moves::Choice;

type ModifyAttackBeingUsed = fn(&State, &SideReference, &mut Choice, bool);

// These need to be changed, they are from the old code
...
lazy_static! {
    static ref ABILITIES: HashMap<String, Ability> = {
        ...
        abilities.insert(
            "analytic".to_string(),
            Ability {
                modify_attack_being_used: Some(|_state, _side_ref, choice, first_move| {
                    if !first_move {
